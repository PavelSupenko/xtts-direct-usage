...
from tts.gpt.gpt_inference import GPT2InferenceModel
from tts.gpt.latent_encoder import ConditioningEncoder
from tts.gpt.perceiver_encoder import PerceiverResampler
...
class GPT(nn.Module):
    ...
    def set_inputs_and_targets(self, input, start_token, stop_token):
        inp = functional.pad(input, (1, 0), value=start_token)
        tar = functional.pad(input, (0, 1), value=stop_token)
        return inp, tar
    ...
    def get_prompts(self, prompt_codes):
        ...
        prompt = functional.pad(prompt, (1, 0), value=self.start_prompt_token)
        prompt = functional.pad(prompt, (0, 1), value=self.stop_prompt_token)
        return prompt
    ...
    def forward(
        ...
        if max_mel_len > audio_codes.shape[-1]:
            audio_codes = functional.pad(audio_codes, (0, max_mel_len - audio_codes.shape[-1]))
        ...
        text_inputs = functional.pad(text_inputs[:, :max_text_len], (0, 1), value=self.stop_text_token)

        # Append silence token to mel codes
        audio_codes = functional.pad(audio_codes[:, :max_mel_len], (0, 1), value=self.stop_audio_token)
    ...
    def compute_embeddings(
        ...
        text_inputs = functional.pad(text_inputs, (1, 0), value=self.start_text_token)
